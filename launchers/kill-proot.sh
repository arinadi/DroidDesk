...
pkill -f "proot-distro login ubuntu" 2>/dev/null && echo "  [x] proot-distro killed" || true

# Kill orphan proot processes tied to the distro rootfs
pkill -f "proot.*installed-rootfs/ubuntu" 2>/dev/null && echo "  [x] orphan proot killed" || true
...
pkill -9 -f "proot.*installed-rootfs/ubuntu" 2>/dev/null || true

# Aggressive Inner Cleanup (Handles residues even if host cleanup fails)
echo "  [*] Performing deep residue cleanup inside proot..."
proot-distro login ubuntu -- bash -c "rm -rf /tmp/xdg-* /tmp/dbus-* /tmp/.xfsm-ICE-* /tmp/.X11-unix/* /home/admin/.cache/sessions/* /home/admin/.ICEauthority /home/admin/.Xauthority 2>/dev/null"
